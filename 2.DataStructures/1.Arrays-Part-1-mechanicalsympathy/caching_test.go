...
// "none" says not to run any test functions, but we want to run all the benchmark functions.
...
// Benchmarking is going to be relative not just to the Hardware but to also what's running in
// the machine. So when we run benchmarks the machine has to be idle.
func BenchmarkLinkListTraverse(b *testing.B) {
	...
	// What bench time mean is that find a value of b.N such that we run this loop for full bench time.
	// In other words we will ask the bench mark function to find b.N that will run for 3 seconds.
...
// This is more to do with the hardware. Mechanical sympathy issue - We recognize that the hardware
...
// In pic 2.png
// When we say we have a 3GHz clock we mean that we get 3 clock cycles every nano seconds.
...
// For every 1 millisecond in latency we are losing appx 12 million instructions.
...
// Everytime we need data and if it is not in one of the caches. We take 100 ns or 1200 instructions hit.
...
// That's where caching was born to get the data closer to the hardware threads.
// Hardware also has special programs running like the "Pre-fetcher".
...
next instruction needed the letter "a" in the word "Main",
we go back to the memory and take 1200 instructions hit again. It could be really bad if
every time we needed the next byte we take 1200 instruction hit loss.
So what happens is different, the granularity is not down to a byte.
What caching system do is, use the granularity of 64 bytes. So we have a 64 byte CACHE LINE.
So when we wanted the letter "M" we will pull the letter "M" and the other 63 bytes. So "ain" of "Main"
are already coming along with it. With cache lines moving on l1,l2,L3 AND L4 we would be taking 1200
...
So how do we make predictable access pattern to memory?
Best way is to allocate contiguous block of memory.
If we can allocate a contiguous block of memory and walk down that memory in a predictable stride.
Pre-fetcher can pick up on that. What data structure allows us to allocate contiguous block of memory?
...
TLB is a little table that is able to convert virtual memory addresses to physical addresses in RAM
by caching operating system pages and offsets of where these virtual addresses are. Remember our software
...
When that happens the Hardware has to ask the OS
...
In a worst case scenario we will have to scan the paging table on the VM. That has to scan the OS paging
table as well. Plus we have the cache line misses. We have to talk about the major latency cost.
It can get very bad. Predictable access patterns help us in both cache lines and the TLB.


Coming back to why the row was faster than column.
...
That is so fast not because of the algorithm but because of the data access.
the pre fetcher is picking up our traversal and is caching these cache lines before we need them.

Column traversal is bad because, the element in row 1 col 1 and the element in row 2 col 1 are in a 
totally separate pages (since our matrix is quite big.) See pic 5.png
so every traversal in column is spanning pages so the TLB cannot be up to date which means we cannot have
...
When we load one cache line we have only 1 out of the 64 bytes that are useful, 
...
Performance today on the hardware that we use is not about pushing the clock, we do not want a 5GHz clock.
...
With row traversal we are as efficient as we can be
...
Array, slice(dynamic array use it to get the dynamic aspect which we do not get our of an array.)
But we get mechanical sympathy of efficiency in getting the data into the processor.
Map also tries to keep the data as contiguous as possible.
...
but as we learnt from the hardware perspective that contiguous memory is always going to be better
so we will take the cost on our stack to allocate a new block to keep the data contiguous 
to give us the best chance of the predictable access pattern.

In the end it is all about the contiguous data, the predictable access pattern,
