/*
	...
	Say we didn't use a buffered channel of 1 but we used an unbuffered channel.
	"ch := make(chan string)"
	In case of unbuffered channel now we need a guarantee that a signal being sent has been received.
	Means sed and receive have to come together and that the receive happens before the send.

	Say we have an unbuffered channel and the go routine is off doing work.
	...
	It's now been 150 milliseconds, and main go routine, signals
	on this case and moves on.
	case <-ctx.Done():
		fmt.Println("work cancelled")
	}

	But the previous go routine hasn't moved on yet. It is still busy
	on this line doing work.
	"time.Sleep(time.Duration(rand.Intn(200)) * time.Millisecond)"
	Then it comes to `ch<-"data"` it wants to signal a result back but there is no go routine to receive it.
	We now have a deadlock as the send cannot finish without the receive.
	There is no receiver insight. We now have a go routine leak. This is very common mistake.
	xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx

	So anytime we do the cancelation one of the first things, that we will look for, is that the channel
	is being constructed as a buffered channel of 1.
	By doing that we guarantee that if the go routine on the receive side walks away because of a timeout
	the go routine on the send side, can still complete and not leak.
	xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx

	This pattern is the one we would use most often because our web services are long running tasks, must have
	time outs, must have cancellations, this is the pattern and how we do it.
