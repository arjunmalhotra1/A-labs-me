/*
	Wait for task - The idea is that we have a go routine, we are going to launch to do some work.
	...
	Remember unbuffered channels mean guarantees, guarantee that the signal being sent has been received.
	So when we look at
	"ch := make(chan string)"
	We will say, we have a channel that has guarantees and we are signalling with the string data.
	Next we create the Go routine
	go func() {
		d := <-ch
		fmt.Println("child : recv'd signal :", d)
	}()
	The go routine does not know, what work to do yet.
	So it is blocked at "d := <-ch", waiting for us to get that work done.

	We get to this point
	That unknown latency is happening here,
	"time.Sleep(time.Duration(rand.Intn(500)) * time.Millisecond)"

	The go routine above is blocked waiting for unknown amount of time for us(main, maybe) to prepare the work.
	Eventually after the wait we prepare the work, and then we perform the send.
	`ch <- "data"`
	There's a guarantee that the receive "d := <-ch" will happen before
	`ch <- "data"` just nanoseconds before.

	In the scenario of "waitForTask()" the sender is going to get the guarantee that the go routine
	that is going to be doing the work has received it.

	This pattern for Pooling.
